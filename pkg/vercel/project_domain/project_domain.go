...
func (h *Handler) Update(dto CreateOrUpdateProjectDomain) (*ProjectDomain, error) {
	// url := fmt.Sprintf("/v8/projects/%s/domains", dto.)
...
func (h *Handler) Delete(projectID, domainName string) error {
	...
	// if err != nil {
	// 	return fmt.Errorf("unable to unmarshal project domain response: %w", err)
	// }
